...
//XTEC ************ AFEGIT - Configuració de l'HyperDB per a XTECBlocs

/** XTECBlocs Configuration: Partitioning **/

/** 
 * wp-config.php must define DB_PREFIX and DB_NUMS
 * DB_PREFIX must be the prefix of the name of all partitioned databases
 * DB_NUMS must be the number of partitioned databases without considering the global database
 */

// a handy function for mapping datasets to databases
function add_additional_databases($db_id,$wpdb){
	$wpdb->add_database(array(
		'host'     => DB_HOST,
		'user'     => DB_USER,
		'password' => DB_PASSWORD,
...
// modularity is calculated using the number of additional databases.
// VERY IMPORTANT! all the queries should reference tables that are in same database.
